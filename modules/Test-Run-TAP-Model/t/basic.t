...
BEGIN { use_ok($m = "Test::Run::TAP::Model") }

isa_ok(my $t = $m->new, $m);
isa_ok($t, "Test::Run::Straps");
...
# These will die in Test::Run::TAP::Model
eval '$t->get_tests()';
ok($@, "Test::Run::TAP::Model dies when calling get_tests()");
eval '$t->run()';
ok($@, "Test::Run::TAP::Model dies when calling run()");
...
isa_ok($t3, $m);
isa_ok($t3, "Test::Run::Straps");

# Call latest_event with a parameter
my $t4 = new $m;
isa_ok($t4, $m);
isa_ok($t4, "Test::Run::Straps");
my %event = ();
$event{type} = 'test';
$event{todo} = 1;
