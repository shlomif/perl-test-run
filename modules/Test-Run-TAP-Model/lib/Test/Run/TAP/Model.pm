...
package Test::Run::TAP::Model;
use base qw/Test::Run::Straps/;

use strict;
use warnings;

use Test::Run::TAP::Model::File;
...
sub new_with_struct {
	...
	$self->{meat} = $meat; # FIXME - the whole Test::Run::Straps model can be figured out from this
...
Test::Run::TAP::Model - Accessible (queryable, serializable object) result collector
for L<Test::Run::Straps> runs.

=head1 SYNOPSIS

	use Test::Run::TAP::Model;

	my $t = Test::Run::TAP::Model->new();

	# Test::Run::Straps methods are available, but they aren't enough.
	...
	# every file is an object (Test::Run::TAP::Model::File)
	...
	# the same as $t1
	my $t2 = Test::Run::TAP::Model->new_with_struct($other_struct);

=head1 DESCRIPTION

This module is a subclass of L<Test::Run::Straps> (although in an ideal
...
There are two ways to access the data in L<Test::Run::TAP::Model>. The complex one,
...
=item L<Test::Run::TAP::Model>

A whole run

=item L<Test::Run::TAP::Model::File>

A test script

=item L<Test::Run::TAP::Model::Subtest>
...
Returns a list of L<Test::Run::TAP::Model::File> objects.
...
This tells L<Test::Run::TAP::Model> that we are about to analyze a new file.
...
=head1 ISA Test::Run::Straps

L<Test::Run::TAP::Model> is a L<Test::Run::Straps> subclass. It knows to run
...
of a hack because I'm not quite sure if L<Test::Run::Straps> has the proper
